    ...
    public class VisualizationLine
    {
        ...
        public void ReCreateVisualLine(Texture2D t, Vector3 start, Vector3 end, float thickness, Color c)
        {
            ...
            for (int k = 0; k < sides; k++)
            {
                ...
                vertices[currentVert + 0] = new VertexPositionNormalTexture() { Position = Vector3.Normalize(m.Forward.Length() > 0.001f ? m.Forward : m.Up) * thickness * 0.5f + start, Normal = axis, TextureCoordinate = new Vector2(k / (float)(sides - 1), 0f) };
                vertices[currentVert + 1] = new VertexPositionNormalTexture() { Position = Vector3.Normalize(m.Forward.Length() > 0.001f ? m.Forward : m.Up) * -thickness * 0.5f + start, Normal = axis, TextureCoordinate = new Vector2(k / (float)(sides - 1), 1f) };
                vertices[currentVert + 2] = new VertexPositionNormalTexture() { Position = Vector3.Normalize(m.Forward.Length() > 0.001f ? m.Forward : m.Up) * thickness * 0.5f + end, Normal = axis, TextureCoordinate = new Vector2(k / (float)(sides - 1), 1f) };
                vertices[currentVert + 3] = new VertexPositionNormalTexture() { Position = Vector3.Normalize(m.Forward.Length() > 0.001f ? m.Forward : m.Up) * -thickness * 0.5f + end, Normal = axis, TextureCoordinate = new Vector2(k / (float)(sides - 1), 1f) };

                vertices[currentVert + 4] = new VertexPositionNormalTexture() { Position = Vector3.Normalize(m.Forward.Length() > 0.0001f ? m.Forward : m.Up) * thickness * 0.5f + start, Normal = -axis, TextureCoordinate = new Vector2(k / (float)(sides - 1), 0f) };
                vertices[currentVert + 5] = new VertexPositionNormalTexture() { Position = Vector3.Normalize(m.Forward.Length() > 0.001f ? m.Forward : m.Up) * -thickness * 0.5f + start, Normal = -axis, TextureCoordinate = new Vector2(k / (float)(sides - 1), 1f) };
                vertices[currentVert + 6] = new VertexPositionNormalTexture() { Position = Vector3.Normalize(m.Forward.Length() > 0.001f ? m.Forward : m.Up) * thickness * 0.5f + end, Normal = -axis, TextureCoordinate = new Vector2(k / (float)(sides - 1), 1f) };
                vertices[currentVert + 7] = new VertexPositionNormalTexture() { Position = Vector3.Normalize(m.Forward.Length() > 0.001f ? m.Forward : m.Up) * -thickness * 0.5f + end, Normal = -axis, TextureCoordinate = new Vector2(k / (float)(sides - 1), 1f) };
                currentVert += 8;
            }
            // indices
        ...
        public void ReCreateVisualLine(Vector3 start, Vector3 end, float thickness, Matrix viewMatrix)
        {
            ...
            for (int k = 0; k < sides; k++)
            {
                ...
                vertices[currentVert + 0] = new VertexPositionNormalTexture() { Position = Vector3.Normalize(bitangent) * thickness * 0.5f + start, Normal = axis, TextureCoordinate = new Vector2(k / (float)(sides - 1), 0f) };
                vertices[currentVert + 1] = new VertexPositionNormalTexture() { Position = Vector3.Normalize(bitangent) * -thickness * 0.5f + start, Normal = axis, TextureCoordinate = new Vector2(k / (float)(sides - 1), 1f) };
                vertices[currentVert + 2] = new VertexPositionNormalTexture() { Position = Vector3.Normalize(bitangent) * thickness * 0.5f + end, Normal = axis, TextureCoordinate = new Vector2(k / (float)(sides - 1), 1f) };
                vertices[currentVert + 3] = new VertexPositionNormalTexture() { Position = Vector3.Normalize(bitangent) * -thickness * 0.5f + end, Normal = axis, TextureCoordinate = new Vector2(k / (float)(sides - 1), 1f) };

                vertices[currentVert + 4] = new VertexPositionNormalTexture() { Position = Vector3.Normalize(bitangent) * thickness * 0.5f + start, Normal = -axis, TextureCoordinate = new Vector2(k / (float)(sides - 1), 0f) };
                vertices[currentVert + 5] = new VertexPositionNormalTexture() { Position = Vector3.Normalize(bitangent) * -thickness * 0.5f + start, Normal = -axis, TextureCoordinate = new Vector2(k / (float)(sides - 1), 1f) };
                vertices[currentVert + 6] = new VertexPositionNormalTexture() { Position = Vector3.Normalize(bitangent) * thickness * 0.5f + end, Normal = -axis, TextureCoordinate = new Vector2(k / (float)(sides - 1), 1f) };
                vertices[currentVert + 7] = new VertexPositionNormalTexture() { Position = Vector3.Normalize(bitangent) * -thickness * 0.5f + end, Normal = -axis, TextureCoordinate = new Vector2(k / (float)(sides - 1), 1f) };
                currentVert += 8;
            }
            // indices
