...
{ 
    ...
    if (tex2D(AlbedoSampler, input.TexCoords).a < 0.001)
    {
        discard;
    }
   ...
   // mer.x = 1;
    ...
    float alpha = tex2D(AlbedoSampler, input.TexCoords).a;
 ...
 //color += tex2D(reflectionSampler, input.TexCoords).rgb;
  ...
  //  color = pow(color, float3(1.0 / 1, 1.0 / 1, 1.0 /1));
    ...
    colorSpec += specularEnv;
     

    float3 indirectDiffuse;
    indirectDiffuse = ambientEnv;
...
float4 MainPSIntermidiate(VertexShaderOutput input) : COLOR
{
 ...
 //   mer.x = 1;
   ...
   /* const float MAX_REFLECTION_LOD = 4.0;
    ...
    return float4(final.xyz, alpha);
}
technique DeferredBlockEffectPTransparent
{
	pass P0
	{
...
technique DeferredBlockEffectDiffuseTransparent
{
    pass P0
    {
        VertexShader = compile VS_SHADERMODEL MainVS();
